...
cargo run --bin hermes -- -c ~/.hermes/config.toml light add "localhost:26357" -c "heliax" -f -p -s "/home/ec2-user/node-heliax/data" -y &>/dev/null

# set the secondary peers for clients on each chain
echo "Adding secondary peers to light client configuration..."
cargo run --bin hermes -- -c ~/.hermes/config.toml light add "localhost:26657" -c "stargate" -s "/home/ec2-user/node-stargate/data" -y --peer-id 2427F8D914A6862279B3326FA64F76E3BC06DB2E &>/dev/null
cargo run --bin hermes -- -c ~/.hermes/config.toml light add "localhost:26657" -c "heliax" -s "/home/ec2-user/node-heliax/data"-y --peer-id A885BB3D3DFF6101188B462466AE926E7A6CD51E &>/dev/null

# add the key seeds to the keyring of each chain
echo "Importing keys..."
cargo run --bin hermes -- -c ~/.hermes/config.toml keys add "stargate" "/home/ec2-user/node-stargate/key_seed.json" &>/dev/null
cargo run --bin hermes -- -c ~/.hermes/config.toml keys add "heliax" "/home/ec2-user/node-heliax/key_seed.json" &>/dev/null
